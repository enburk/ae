...
namespace pix
{
    ...
    {
        typedef T type; std::vector<type> data; XY size; rectifier update;
	    ...
	    void resize (XY Size) { data.resize (Size.x * Size.y); size = Size; update = XYXY(*this); }

        void fill (const type & c) { for (auto & v : data) v = c; update = XYXY(*this); }
    };
